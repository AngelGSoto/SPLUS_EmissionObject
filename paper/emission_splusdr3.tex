...
Hierarchical clustering also known as hierarchical cluster analysis (\texttt{HCA})
belong to the family of clustering algorithms on which are constructed cluster
by merging clusters and splitting them successively. {\sc hca} is an unsupervised
technique. It consists of building a binary merge tree, starting from the data
elements stored at the leaves (interpreted as individual cluster)
...
This technique is also called \textit{agglomerative hierarchical clustering}
since the individual data point are successively agglomerated into higher-level
clusters. In every iteration, two cluster are selected that are considered as close as
possible. These cluster are merged and replaced with a newly created merged cluster.
Thus, each merging step reduces the number of cluster by ``1''. Therefore, the
method needs to be designated for measuring proximity between cluster containing
multiple data points, so that they may be merged \citep{Aggarwal:2015, Mann:2013}.
We could describe how to work the algorithm in three simple steps:
