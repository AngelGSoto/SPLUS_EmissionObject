...
$^{1}$Departamento de Astronomia, IAG, Universidade de S\~{a}o Paulo, Rua do Mat\~{a}o, 1226, 05509-900, S\~{a}o Paulo, Brazil\\
...
classification of variable stars, relying only on the similarity among light curves. 
\citep{Valenzuela:2018}. Using unsupervised machine
...
attempt to find the loci of main-sequence and giant stars. The objects
with a excesses the H{$\alpha$} 
To obtain a better
We also implemented

We considering the best-fitting as the main stellar locus

Applying the selection criteria to selecting H$\alpha$ emitters.
We used similar procedure in  \citet{Wevers:2017}. The objects
...
the fit and $\sigma_{\mathrm{phot}}$ is the error on the observed $(r - J0660)$ colour.
...
The idea is that we considered those objects 
...
\begin{figure*}
  ...
  \caption{An illustration of the selection criteria used to identify strong
    ...
    fit, it is shown in red (blue). Objects selected as H{$\alpha$} emitters must be located above
...
Once, we felt confident of our sample of H{$\alpha$} emission lines, we proceeded
in classify the objects in to big groups; those with strong blue continuum and another
with intense red continuum. 
...
two cluster that in agreement with previous results are the blue and red population.
