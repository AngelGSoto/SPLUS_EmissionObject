...
\item The clusters are extracted on the notion of persistence in the hierarchy.
  ...
  cluster (or was split off into new cluster), $\lambda_{\mathrm{birth}} \leq \lambda_{p} \leq \lambda_{\mathrm{death}}$. Cluster stability  $S$ is defined as the sum of $\lambda_{p} - \lambda_{\mathrm{birth}}$ for all points in the cluster. To extract cluster the following procedure is
...
\newcommand\TableHeader{
  \hline\hline
  Id Simbad & \(\mathrm{RA}\) & \(\mathrm{Dec}\) & Type & Group -- {\sc hac}
