...
Here we present H$\alpha$ emitters from the S-PLUS iDR3 by using color-color diagrams and unsupervised machine learning techniques, classifying them as blue or red fonts and also proposing a class to which they belong. [** Section XX describes YY, section XX, section XX and finally section XX discusses our study and conclusions.]
...
\rlopes{This paper uses data from S-PLUS, a photometric sky map being carried out since end of 2017 using a 0.83m robotic telescope located at Cerro Tololo, Chile \citep{Mendes:2019}.}
\rlopes{The project is surveying the Southern sky with 12 filters from the Javalambre filter system \citep{Martin-Franch:2012} spanning the wavelength range from 3000\AA\ to 10000\AA}. The filters used are \rlopes{seven narrow-band filters}
(\textit{J}0378, \textit{J}0395, \textit{J}0410, \textit{J}0430,
\textit{J}0515, \textit{J}0660,  and \textit{J}0861) 
\rlopes{and}
\rlopes{five broad-band sloan-like \citep{Fukugita:1996} filters (\(u, g, r, i~\mathrm{and}~z\))}.
The narrow-band \rlopes{$J0660$ filter}  \rlopes{covers} 
the H{$\alpha$} plus \rlopes{the} [N II] \rlopes{spectral} lines \rlopes{for sources up to redshift of approximately 0.02 \citep[see Fig. 1 after][]{Martin-Franch:2012}}. 
...
\rlopes{The first goal of this paper is to identify objects that may be bona-fide H$\alpha$ emitters among sources detected in the S-PLUS DR2.}
 \rlopes{For this,} we
applied an iterative and automatic technique to select objects with
an excess \rlopes{in the $J0660$ band, which is consistent with the detection of the H{$\alpha$} line in emission.} 
\rlopes{Next, the selected objects were separated}
into two groups\rlopes{, one having redder and the other bluer emission.} 
\rlopes{This classification was done} 
...
\rlopes{in the following sections.}
...
%\sout{The first step in our selection procedure consists of the following
...
\rlopes{We first generated the ($r$ - $J0660$) versus ($r$ - $i$)}
\rlopes{diagram for each magnitude bin and attempted to put in evidence the loci mainly occupied by main-sequence and giant stars with a linear regression fit} \comment{(CORRETO?)}, \luis{sim}. 
\rlopes{We then implemented an iterative $\sigma$-clipping technique so that, by construction, H$\alpha$ emitters should satisfy the condition:} \comment{(CORRETO?)} \luis{Acho sim, o objetivo de fazer um iterativo sigma clipping e encontrar o melhor ajuste posivel}
...
The fits \sout{are} \rlopes{were} carried out with the aid of the python library. \texttt{astropy.modeling}
\footnote{https://docs.astropy.org/en/stable/modeling/index.html}

Figure~\ref{fig:criteria-color-plot} illustrates the procedure \rlopes{applied}. The continuous black lines represent the initial
...
fit. Note that these cut lines are only an approximations because \rlopes{only} the residual around the fit \rlopes{is taken into account}. 
\rlopes{Note that the approach includes} the phothometric uncertainties of the ($r$ - $J0660$) color for each individual data source \rlopes{(see Equation~\ref{eq:criterion})}.
...
\rlopes{Once having selected H$\alpha$ emitters, we proceed with a visual inspection of their spectral energy distributions as seen from the whole S-PLUS coverage, namely S-spectra, and their false-color images.} 
...
\rlopes{For illustration, Figure~\ref{fig:Spectra} shows how an H$\alpha$ emitter looks like from the S-PLUS data.}
...
\rlopes{Figure}~\ref{fig:emission} exhibits the distribution of the H{$\alpha$} emitters on the
($r - J0600$) versus ($r - i$) color-color plane. 
\rlopes{It also present the main-sequence and giant stars loci as derived from synthetic spectra collected of \citet{Pickles:1998} convolved with the transmission of S-PLUS filters, and defined in the AB magnitude system \citep{Oke:1983}.}
%\sout{
%,
...
\rlopes{Objects of our sample were}
divided 
into two groups,
\rlopes{distinguishing the bluer from the redder population}.
\rlopes{To do this, we apply an unsupervised machine learning approach to implement}
 two clustering techniques\rlopes{:} hierarchical clustering and hierarchical
density-based cluster selection, \rlopes{both} based on the $(g - r)$ and $(z - g)$ colors \rloes{, whose results were mutually compared}. 

\subsubsection{Hierarchical agglomerative clustering}
\label{sec:Hierar}

Hierarchical clustering belongs to the family of clustering algorithms
\sout{on} \rlopes{of} which \rlopes{clusters} are constructed  by \rlopes{grouping} and splitting \rlopes{the sources} \rlopes{in a recursive way}.
...
\rlopes{which} represents the nested grouping based on patterns and the levels of similarity
at which the groupings change \citep{Jain:1999}. There \rlopes{are} two \rlopes{types} of hierarchical
clustering: 
 the \textit{hierarchical agglomerative clustering} (that we have
used in this work), which is ``bottom-up'' approach, \rlopes{and the \textit{hierarchical divisive clustering}, with a ``top-down'' approach. The} hierarchical agglomerative
...
of the data set. The agglomerative term is used \rlopes{because} individual data
 are successively agglomerated into higher-level. In each iteration, two \rlopes{``nearby'' clusters}
are selected 
...
\rlopes{and collapsed them into a new, more populated group \citep{Mann:2013, Aggarwal:2015}.}
 Thus, each merging step reduces the number of cluster by a factor of ``1'' \comment{(Eu não entendi esse 1)}. 
...
\rlopes{The procedure may be summarized in three steps:}
...
\item Through a looping process, the \rlopes{clusters} or nodes are merged into groups that have the maximum similarity between them.
\item At the end of the process\rlopes{,} all the nodes belong to an unique cluster. This is
  known as the root of the tree structure.
 \end{enumerate}

\rlopes{In the opposite direction is the other type of hierarchical clustering,} the \textit{hierarchical divisive clustering}. \rlopes{Following its ``top-down'' approach, the investigation starts from data as being in one only cluster then moving down in the hierarchy to small groups in a recursive way.}
...
\rlopes{algorithms intend to} 
\rlopes{gather} similar objects into groups called clusters or nodes \rlopes{in the space of parameters which is investigated}. 
...
\comment{Eu não entendi o que você quer dizer com essa frase:} It is based on a slightly modified version of density-based spatial
...
by ``islands'' of high density in the sea of the parameter space \rlopes{. Such} clusters \rlopes{constitute} data partitions that have a higher density
...
clusters of variables densities and different \rlopes{shapes}. Following \citet{Malzer:2021}
and \citet{Ntwaetsile:2021}\rlopes{, it} works as follows:

\begin{enumerate}
\item \rlopes{\texttt{HDBSCAN} define} the ``core'' distance for a point $x$, core$_k(x)$,
  \rlopes{as} the distance of an object to \rlopes{its} $k$th nearest neighbor. This mean that \rlopes{lower} values of core$_k(x)$ 
  \rlopes{represent higher densities} and vice-versa.
...
\item The ``mutual readability distance'' between two points $a$ and $b$ is defined as
  ...
  where \comment{d(a,b) ou dm(a,b)?):} $d(a, b)$ is the distance between $a$ and $b$ according, for instance,
  Euclidean metric. The mutual readability distance allows points in dense regions
  to stay close together and those that are in less dense regions pushed away \comment{(N\~ao ficou claro para mim.)}.
...
\comment{Eu preferi n\~ao mudar nada para os itens acima porque eu não entendi tudo e corro o risco de mudar o sentido de alguma coisa.}
...
\rlopes{We adopt the $g$, $r$, and $z$ broad-bands spreading from the blue to the red in the optical to distinguish the blue from the red populations of our sample. In this way, we start by exploiting the $(g - r)$ and $(z - g)$ colors and the potential of the corresponding diagram to put in evidence different classes of systems.} \comment{Eu recomendo uma mudan\c ca na apresenta\c c\~ao, come\c cado com um texto assim... depois migrando para falar que populamos o diagrama com fotometria sint\'etica para certos tipos de objetos e calculadas via espectros observados para outros tipos de objetos. E s\'o depois entrar na descri\c c\~ao que est\'a sendo feita no in\'icio da pr\'oxima se\c c\~ao - dizendo onde cada classe cai em tal diagrama. O que acha? E desse modo o t\'itulo da subse\c c\~ao 3.4.1 pode ser eliminado.}
...
The ideal way to choose the number of clusters can be done by displaying the \textbf{dendrogram diagram}. Firstly, the hierarchical cluster output dendrogram (tree; see for instance Figure~\ref{fig:dendrogram}) can be implemented to obtain the desired clustering. Secondly, the dendrogram schema allows 
...
include in the python package \texttt{scipy}\footnote{https://www.scipy.org/}, allows us to generate this diagram, and has a feature to allow a better visualization of them. 
...
Comparing with These spectra mainly correspond to HII regions, cataclysm variables, SN, emission-line galaxies (blue compact galaxies, H II galaxies, star-forming galaxies, among others), AGN (Seyfert 1 and 2), and QSOs. However, more detailed analyzes are necessary to see that other possibles types 
...
in our list, including nebulae, binary systems, variable stars, x-ray sources, emission line galaxies, quasars, globular clusters, among other. Match with spectroscopic database (SDSS and Lamost) evince that mostly of the objects selected are emission line objects, showing  that at least the XX\% of the objects are genuine H$\alpha$. 
The big advantage here compared to previous work is that we are providing a significant sample of H $\alpha$ emitters in 12 bands. Seven and five narrow- and broad-band filters, respectively, covering the wavelength range from $\sim$3000 to $\sim$9000\AA. In other words, much more information can be extracted from them in future work only with S-PLUS photometry. 

The $g - r$ and $(z - g)$ distributions of the H{$\alpha$} emitters are bimodal, evidencing two populations.  The blue peak corresponds mainly to blue CV, QSOs, PNe, dwarf compact galaxies, H II regions, among other. The read peak is due to early type galaxies with emission lines (for instances, radio-galaxies and Seyfert 2 galaxies) and probably young/active late-type stars.
Agreement to this results the 
