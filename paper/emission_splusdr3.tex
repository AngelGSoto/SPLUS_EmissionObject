...
The first step in our selection procedure consists in the following
...
\item The sources must have detection in the filters: $r$, $i$ and
  $J0660$. To assure that, we select object must have error minor or
...
an excess of H{$\alpha$} which is represented with a relatively excess magnitude-value  of
...
implemented by \citet{Witham:2008}. It is possible to do that because
 S-PLUS has similar filters to the IPHAS project, which are
...
also include the phothometric uncertainties in ($r$ - $J0660$) color for each individual data source as shows on the Equation~\ref{eq:criterion}.
...
spectral energy distributions of \citet{Pickles:1998}. This synthetic magnitudes are
...
or source with strong blue continuum as cataclysm variables and QSOs.
...
Figure~\ref{fig:aitoff-distribution} displays the distribution of all emitters in Galactic la
...
continuum on the red part of the spectrum. 
...
into two groups: one representing the blue sources and the another the red sources.
...
compare the results between them. Given 
that these colours will be the input of the machine learning approaches implemented here,
we first will speak about them.
...
\begin{figure}
	\includegraphics[width=0.9\linewidth]{Figs/Fig-SPLUS-gr-zg.pdf}
        \caption{The $(g - r)$ versus $(z - g)$ synthetic color-color diagram of
         ...
         a subjective criterion to separate the objects into two color types. {\sc Maybe 
...
The ideal way to choose the number of clusters can be done by displaying the dendrogram diagram. Firstly, the hierarchical cluster output dendrogram (tree) can be implemented to obtain the desired clustering. Secondly, the dendrogram schema allows
...
\begin{itemize}
     ...
     \item The $y$ axis represents the distances of the ``ward'' method in our case.
       
\end{itemize}


\paragraph{Dendrogram Truncation}
...
some other features of the \textsc{dendrogram()} function:
...
declare points as noise. It is a algorithm that assumes clusters that are characterized
...
(e) & (f) \\
  ...
  \caption{Summary of our selection results showing the spectrum (gray line en each panel) of 
          ...
          a cataclysm variable star (\textit{c}), a super nova (\textit{d}), a WR in a galaxy (\textit{e}) 
          and a QSO (\textit{f}) with red-shift of $\sim$ 2.45. 
...
\includegraphics[width=0.5\linewidth, trim=10 10 10 8, clip]{Figs/spec-0283-51959-0496-SPLUS-n01s20-026220.pdf}  \llap{\raisebox{3.7cm}{\includegraphics[width=0.2\linewidth, trim=200 10 300 30, clip]{Figs/Galaxy_176.35960159169613-0.0041113183584209_150_r.pdf}}} &
  ...
  is responsible for the $J0660$ magnitude. And a Seyfert 1 with $z = 0.317$ (\textit{d}). For this last object, the excess on the $J0660$ is due to the [O III] 4959, 5007 \AA~ emission lines (\textit{d}). As in Figure~\ref{fig:photo-image-knonw-objects} coloured symbols indicate the S-PLUS photometry.}
