...
at which the groupings change \citep{Jain:1999}. There is two type of hierarchical clustering:
one is the \textit{hierarchical agglomerative clustering} (that we have used
in this work) which is ``bottom-up'' approach. Hierarchical agglomerative
clustering (\texttt{HAC}) consists of building a binary merge tree, starting from the
data elements stored at the leaves (interpreted as individual clusters)
...
Agglomerative term is used  to define it since the individual data
...
\subsection{Applying the approaches to  the H{$\alpha$} list}
\label{sec:apply-hac-hdebscan}

In this work, we implemented {\sc hac} by using the python machine learning library
\texttt{Scikit-learn} \citep{Pedregosa:2011}. There are some parameters to have in count
where the algorithm is apply: \texttt{n\_clusters} which is the number cluster to find. Given
...
The algorithm will merge the pairs of cluster that minimize this criterion. The implemented here
was ``ward'' which minimizes the variance of the clusters being merged. Fig.~\ref{fig:dendrogram}
shows the dendrogram truncated diagram visualization that shows the branching of the sample from the main root.
