...
\rlopes{In the way to map 9000 deg$^2$ of the Southern hemisphere, the S-PLUS project is also surveying the sky in a proxy of a myriad of astrophysical processes: the H$\alpha$ transition. Here we explore such a capability from its DR3 to make H$\alpha$ emitters in evidence from 
...
falls into the $J0660$ filter. Objects with $ z \gtrsim  0.02$ the  H{$\alpha$} is outside 
of the $J066o$ filter. Table~\ref{tab:simbad} of the appendices shows the red-shift of the
emission line galaxies of our sample that have matches on SIMBAD, probing that mostly of 
the galaxies are real H{$\alpha$} emitters. In these kind of galaxies such as the blue 
compact and/or H II galaxies their starburst regions that consist of ensemble of massive ionizing 
stars and their respective giant H II regions cover the extension of the optical images and 
the emission lines of their spectra.

On the other hand, Seyfert 1 and Seyfert 2 galaxias together with the 
...
(c) & (d) \\
  ...
  \end{tabular}
  \caption{SDSS spectra of a HII galaxy ((\textit{a}), a radio galaxy (\textit{b}), 
  a seyfert 1 (\textit{c}) and a star-forming galaxy (\textit{d}). As Figure~\ref{fig:photo-image-knonw-objects}, coloured symbols indicate the S-PLUS photometry.}
