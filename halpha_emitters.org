...
#+BEGIN_SRC python :return filename1 :return filename2 :return filename3 :results file
  ...
  table_blue = Table.read("iDR3_n4/Blue0-Good-LD-Halpha-DR3_noFlag_merge-takeoutbad-Final.ecsv", format="ascii.ecsv")
  table_red = Table.read("iDR3_n4/Red1-Good-LD-Halpha-DR3_noFlag_merge-takeoutbad-Final.ecsv", format="ascii.ecsv")
  ...
  sns.despine()
  plt.tight_layout()
  filename1="paper/Figs/distribution-Halpha-group.pdf"
  ...
  sns.distplot(r_iblue, 
	norm_hist=True, kde=True, ax=ax2,
	bins=40, hist_kws=dict(range=[-3.0, 3.0], color='b')
		)
  sns.distplot(r_ired, 
	norm_hist=True, kde=True, ax=ax2,
	bins=40, hist_kws=dict(range=[-3.0, 3.0], color='r'),
		label="Red")
  ax2.set(xlim=[-2.0, 2.5])
  #ax2.legend(loc='upper right')
  ymax = ax2.get_ybound()[1]
  sns.despine()
  plt.tight_layout()
  filename2= "paper/Figs/distribution-ri-group.pdf"
  ...
  sns.despine()
  plt.tight_layout()
  filename3="paper/Figs/distribution_r-group.pdf"
  plt.savefig(filename3)
  
#+END_SRC  

#+RESULTS:
[[file:paper/Figs/distribution_r-group.pdf]]
...
[[file:Index(['Field', 'ID', 'RA', 'DEC', 'FWHM', 'ISOarea', 'KRON_RADIUS', 'MU_MAX',
       ...
       'z', 'angDist'],
      dtype='object')
Number total of objects is, 1041
...
#+RESULTS:
[[file:Final-list/QSO-simbad-final.csv]]
...
#+BEGIN_SRC python :results file :noweb yes 
   ...
   with sns.axes_style("ticks"):
       ...
       ax1.annotate(r"$z = 0.02$", xy=(0.08, 1.9), alpha=0.9, size=16.0,
                   xytext=(0.1, 170), textcoords='offset points', ha='right', va='bottom', 
...
#+RESULTS:
[[file:Final-list/color-mag-gaia.pdf]]
...
#+BEGIN_SRC python :return pltfile :results file :results output
  ...
  # removing inf or nan values
  col = ["rgeo", "phot_g_mean_mag", "bp_rp"] 
  df_new = df_dis[col] 
  df_new1 =  df_new.dropna()

  # G-mag
  Gmag_abs = np.array(df_new1["phot_g_mean_mag"]) - 5*np.log10(np.array(df_new1["rgeo"])) + 5
  ...
  # QSO
  Gmag_qso_abs = df_qso_dis["phot_g_mean_mag"] - 5*np.log10(np.array(df_qso_dis["rgeo"])) + 5
  bp_rpmag_qso =  df_qso_dis["bp_rp"]
  colors = ["cerulean",]
  colors = sns.xkcd_palette(colors)

  pltfile = 'color-mag_abs-gaia-distance.pdf'
  sns.set_style('ticks')
  fig = plt.figure(figsize=(6, 7))
  ax = fig.add_subplot(111)
...
#+BEGIN_SRC python :return pltfile :results file :results output
  ...
  # QSO
  d_qso = np.array(df_qso_dis["rgeo"]) / 1000.
  bp_rpmag_qso =  df_qso_dis["bp_rp"]
  colors = ["cerulean",]
  colors = sns.xkcd_palette(colors)
...
#+BEGIN_SRC python :return pltfile :results file :results output
  ...
  d_qso = df_qso_new1["rgeo"] / 1000.
  
  pltfile = 'dist-distance-Bailer-gaia.pdf'
  with sns.axes_style("ticks"):
    # Bar diagram
    fig, ax1 = plt.subplots(1, 1, figsize=(10, 6), sharex=True)
    plt.xlabel(r"$D(Kpc)$", fontsize=33)
